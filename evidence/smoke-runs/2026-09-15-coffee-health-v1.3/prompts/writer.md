...
Run folder: /Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3   Mode: report

**Length: 600 words before the Sources list, and the ceiling is enforced by the citation script.** Write to the target, not past it: before drafting, write an outline with a word budget per section that sums to the target (see "Outline first"), then draft each section to its budget. Do not draft long and cut; the previous writers wrote 2,800-3,400 words and then spent a third of their budget trimming.

Write `/Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3/report.md` from the assembled evidence only. Inputs: `/Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3/00-brief.md`, the claim list below, `/Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3/sources.md`. Do not read raw pages, do not search, do not add facts from memory. Every factual sentence carries a citation `[n]` whose number exists in `sources.md`; the ledger assigned those numbers and you may not invent, merge or renumber them.

## Claims (label; supports; contradicts)
Read the full claim list by running: `python3 /Users/lzh/Desktop/ResearchWorkflow/skill/deep-research/scripts/ledger.py --run /Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3 claims list --format md` (all labels). Also read `/Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3/verification.md`.

## Outline first (required)
Before any prose, write `/Users/lzh/Desktop/ResearchWorkflow/evidence/smoke-runs/2026-09-15-coffee-health-v1.3/outline.md`. **First, a question-coverage table:** one row per numbered part or explicit sub-question of the research question (and of the brief's "sub-questions to watch"), with the claim ids that answer it; a row with no claim, or only a claim that answers it in part, is written as a gap, goes into "What this report could not find", and is named in your final message so the main agent can decide on one more research pass. In the September 2026 runs the rubric items missed twice were passages on pages that had been fetched but that no claim covered (the review standards that survive a holding; a guideline's companion recommendation); this table is where that shows before the report is judged. **Then** the section list below, one line per section with its word budget and the claim ids it will use, budgets summing to the length target (for a 1,500-word target, roughly: summary 150, findings table 200, body sections 900 shared by theme, disagreements 120, could-not-find 80, methodology 50). Put every corroborated central claim in a section of the outline; if they do not fit the budget, merge claims that say the same thing into one sentence rather than dropping them. **If the question or the brief asks for a presentation element** (a dated timeline, a comparison table, a glossary, a ranked list; look in the brief's "Sub-questions to watch" and its question line), give that element its own section in the outline with its own word budget and its own claim ids, and produce it in the report as that section (a timeline is a dated table, one cited row per event; a comparison table has one row per item), placed after the findings table unless the brief says where. Do not fold it into the prose sections: on 2026-09-10 a writer read the brief's line about a timeline, wrote the dates into prose, and the report lost the rubric's presentation item until a second pass added the table. Then draft section by section to the budget. Count words as you finish each section and shorten that section before moving on, not the whole draft at the end.
