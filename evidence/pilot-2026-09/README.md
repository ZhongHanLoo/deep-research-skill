...
## Skill v1.3 confirmation runs on the same briefs (2026-09-11 afternoon): science-health-2 and business-finance-3

Three changes were applied on 2026-09-11 (progress #63; `decisions/backlog.md` package): a researcher-prompt line that registers a page's own definitional, status, threshold and rule sentences as `supporting` claims (candidate C1 from `decisions/adjacent-passage-class.md`, whose offline test registered the missed sentence on 4 of 5 cached pages against 2 of 5 for the unchanged prompt), a stop-rule clause that a page counts once any claim is registered from it, and a judge-prompt line that an item with several parts passes only if every part is present. Contract unchanged. Two confirmation runs repeated earlier questions with their briefs copied verbatim (round-2 angles included): the morning's post-cutoff public-health question, and the stable deposit-and-brokerage-protection question from 2026-09-10 whose one miss was the exact sentence the C1 line was written for. Each earlier report was re-judged with the new judge line so both sides are scored the same way (both re-judges reproduced the original verdicts item by item).

| | v1.3 | v1.2, same brief, same day, re-judged |
|---|---|---|
| Compliance | **1.00** first pass, no coverage pass | 0.929 first pass (the re-judge reproduced the morning's verdicts item by item) |
| Generation tokens | 1.73M (research 610k / verification 899k / writer 221k) | 1.46M (669k / 622k / 169k) |
| Agents (killed) | 15 (0) | 13 (0) |
| Sources / claims / central | 63 / 136 / 53 | 47 / 122 / 44 |
| Corroborated / single-source / contradicted | 46 / 89 / 1 | 39 / 83 / 0 |
| Report words, first draft → final | 2,359 → 1,592 | 1,805 → 1,722 |
| Citation checks | 136/136 quotes, 128 citations, 0 errors | 122/122, 86 citations, 0 errors |
| `cite_audit.py`: validity / containment | 0.92 / 0.36 (one URL in the source table flagged fabricated: a researcher's guessed press-release address, registered `unreliable`, never fetched or cited in the text) | 0.92 / 0.47 |
| Wall clock | 57 min | 45 min |

| | v1.3 (business-finance-3) | v1.2 + edits, 2026-09-10, re-judged |
|---|---|---|
| Compliance | **0.84** first pass and after one coverage pass | 0.92 |
| Generation tokens | 1.73M (research 628k / verification 907k / writer 195k) | 1.87M |
| Agents (killed) | 15 (0) | 17 (0) |
| Sources / claims / central | 64 / 137 / 60 | 75 / 169 / 73 |
| Corroborated / single-source / contradicted | 55 / 82 / 0 | 66 / 103 / 0 |
| Report words, first draft → final | 1,928 → 1,726 (1,749 after the pass) | not separable |
| Citation checks | 137/137 quotes, 146 citations, 0 errors | 0 errors |
...
## Experiment after v1.3: two writer-prompt changes tested offline on one ledger (2026-09-15)

The v1.3 confirmation on the deposit-and-brokerage question had left the insurer's status sentence registered but unused by the writer, and both v1.3 writers had drafted 28-55% over the length target. Two writer-prompt passages were designed (`decisions/backlog.md` W1: at most one claim id per 20 words of a section's budget and a per-section word check; W3: a "what it is / status" row per named entity in the outline's coverage table, plus a self-check pass over those rows) and tested offline on two scratch copies of that run's ledger, one writer per arm, the control on the tagged prompt, the treatment with the four passages inserted (`eval/experiments/writer-w1w3.md`), one Opus judge each, 0.34M Sonnet and 0.13M Opus. Result: control 0.92 (one miss: the second body's status, which no claim carries), treatment 0.84 (the same miss, plus a three-part recall item compressed to two parts). Both arms cited the insurer's status claim in a comparison-table row, the control unprompted, so the 2026-09-11 omission was writer variance rather than something only the new row reaches. The treatment's status rows did surface the missing status as a declared gap, but the report then filled the table cell without a claim, which the judge flagged as an internal contradiction. The density rule was not obeyed at the outline (15-19 ids on 200-word sections); the treatment drafted closer to target (12% over on first assembly against 25%) but left 14 corroborated central claims unused against the control's 6 and the live run's 0. Neither passage is applied. One sample per arm on one ledger, where the unchanged prompt scored 0.84 on 2026-09-11 and 0.92 on 2026-09-15.
