# deep-research — file and CLI contracts (v1.1, 2026-09-10; v1 2026-09-03)
...
<run>/                         created by `ledger.py init`; default root `research-runs/`, name `<YYYY-MM-DD>-<slug>`
  ...
  outline.md                   written by the writer agent before drafting: one line per section with its word budget and claim ids (2026-09-10)
...
- `grade` (model-assigned source quality, set by whichever agent registered the source: the researcher after extraction, the verifier right after `add-url`; added 2026-09-10 because verifier-added sources carried no grade in the pilot and the judge criticised low-grade corroboration): `primary` | `secondary` | `blog` | `forum` | `unreliable` | null
- `health` (set by cite_check.py): `LIVE` | `DEAD` | `ARCHIVED-ONLY` | `POSSIBLY-FABRICATED` | `UNKNOWN` (not live now and the archive lookup itself failed; recheck later) | null
...
Plausibility gate (survey §4.2) is applied to every rung's text: length < 200 chars → `failed:length`; block-page markers in first 5000 chars (`attention required! | cloudflare`, `sorry, you have been blocked`, `please verify you are a human`, `checking your browser`, `anubis uses a proof-of-work scheme`, `enable javascript and cookies`, `access denied`, `request blocked`) → `failed:block-page`; consent markers dominating (`before you continue`, `we use cookies and data`, `manage your privacy settings`, `accept all cookies` and text < 1500 chars) → `failed:consent-wall`; text:HTML ratio < 1.5% when HTML ≥ 20 kB → `failed:js-empty`; `%PDF-` prefix or ≥2 of `endobj|endstream|/FlateDecode|xref|trailer` → `failed:unextracted-pdf`; ≥200 000 chars with < 1 sentence terminator per 5000 chars → `failed:word-list`; > 5% replacement/control characters in the first 5000 chars → `failed:binary` (added 2026-09-03: undecoded gzip bodies from Wayback passed the gate); **nav-only** (added 2026-09-10): strip Markdown images, links (keep the link text) and bare URLs; a line is *navigation* when, after stripping list bullets and heading marks, it has fewer than 8 words and does not end with a sentence terminator; *prose words* are the words inside sentences of ≥ 40 characters (a run of text ending in `.`, `!` or `?`) in the remaining lines, joined per blank-line paragraph; the text fails with `failed:nav-only` when navigation lines hold ≥ 85% of the characters **and** prose words < 120. Calibrated on the 429 `ok` sources of the September 2026 pilot: it flags exactly three (a menu-plus-"page no longer exists" British Museum record, a Supreme Court of Canada item page with 6 prose words, a Europe PMC page with none) and passes documentation and journal pages whose reader-proxy text is mostly sidebar (MDN, JAMA, FINRA: 145-264 prose words). A failed gate advances the chain; the reason is recorded in `attempts`.
...
- `importance`: `central` | `supporting` | `tangential` (extractor's judgment relative to the research question). Claims are atomic: one fact, ≤25 words; several claims may share one quote. **Mechanical caps:** (2026-09-03) `ledger.py` writes at most `CENTRAL_CAP` (default 4, env `DEEP_RESEARCH_CENTRAL_CAP`, 0 disables) central claims per source; (2026-09-10) and at most `ANGLE_CENTRAL_CAP` (default 16 = a source target of 4 × the per-source cap, env `DEEP_RESEARCH_ANGLE_CENTRAL_CAP`, 0 disables) central claims per angle slug, counted across rounds, so the run-level bound is angles × 16 whatever the researcher fetched. Further central claims are stored as `supporting` with note `importance-capped: …` (the note names the cap that bound) and reported in the `claim add` output (`capped_to_supporting`, `note`). Evidence: the per-source cap alone did not bind per run while researchers fetched 7-8 sources against a target of 4 (central 100 and 86, progress.md #38); the source-target stop rule brought central to 59 with no loss of recall (#39); the per-angle cap is the mechanical backstop for that rule. `state` reports central claims per angle.
...
| `add-url URL [--angle A] [--round R] [--title T] [--ignore-robots] [--fresh]` | Assigns the next `n`, runs `fetch.py` (same directory, via `subprocess`), stores `raw/<n>.txt` + `raw/<n>.meta.json`, appends the row. If the URL (normalised: lowercase host, strip `www.`, strip fragment, strip trailing slash, strip `utm_*`; **known mirrors fold to one key**: any URL whose work key is an RFC number (rfc-editor.org, datatracker/tools/www.ietf.org, httpwg.org, greenbytes.de, hjp.at; the same rule as the independence check) keys as `rfc-editor.org/rfc/rfcN.html`; `xx.m.wikipedia.org` keys as `xx.wikipedia.org`; the stored `url` stays as given; added 2026-09-10 after one run registered RFC 9111 three times from three hosts, #45) already exists, returns the existing row without refetching. Prints `{n, status, text_path, extracted_chars, evidence_strength, quote_safe, fetch_method, title, published}`. |
...
| `grade N --grade G [--published DATE] [--publisher P]` | Sets the model-assigned grade and optional metadata. An omitted option leaves its field as it is; an **empty string clears it** (`--published ""` sets `published` to null, `--publisher ""` sets `publisher` to null), which is how the main agent repairs a placeholder or wrong date an agent reported (added 2026-09-10 after two runs needed the field cleared by hand, #43). Prints `{n, grade, published, publisher}`. |
...
| `claim add --from-json FILE [--round R]` | Same for a JSON array of `{source, angle, text, quote, importance, round?}`; validates all, writes the valid ones, reports rejected ones by index; exit 3 if any were rejected. A claim's `round` is, in order: the item's `round`, then `--round`, then the source's `round`, then 1 (2026-09-10: before, an item without `round` took its source's round, so eight round-2 claims on a round-1 source fell between the two batch builds, #39). |
...
| `claim unevidence ID --source N [--by LABEL] [--uncheck]` | Removes the `supports`/`contradicts` entries for source `N` (only those with that `by` label when given); appends note `unevidence: [N] removed by …`; recomputes the label; with `--uncheck`, resets `checked` to false when no evidence remains. Exit 2 when nothing matched. The only ledger undo; for repairing placeholder or wrong registrations from the main session (added 2026-09-10 after a placeholder evidence entry had to be removed by hand, #37). |
...
2c. **Coverage**: central claims labelled `corroborated` or `single-source` whose id appears in no marker are listed as `central-claim-unused` (info) and counted in the summary (added 2026-09-03 after the Stage B judge failed two rubric items whose facts were in the ledger). Supporting claims with the same labels whose id appears in no marker **and whose text or quote carries a number, a percentage, a grade or a recommendation** (digit, `%`, `grade`, `recommend`, `MUST`, `SHOULD`, `shall`) are listed as `supporting-claim-unused` (info) and counted separately (added 2026-09-10 after two rubric facts sat in unused supporting claims that the central-only list did not show, progress #47).
...
3. **URL health** (skipped with `--no-network`): for each source with `status == ok` or cited in the report: HEAD (fallback GET, 15 s) → 2xx/3xx = `LIVE`; 4xx/5xx/timeout → CDX exact query; ≥1 capture → `ARCHIVED-ONLY`; no capture and DNS failure → `POSSIBLY-FABRICATED`; no capture otherwise → `DEAD`; **CDX error (429/5xx/timeout) → `UNKNOWN`** and a `health-unknown` warning, never `DEAD` (added 2026-09-04: with four parallel workers the CDX endpoint rate-limited and 13 live-fetched, bot-walled sources were reported dead). Sources whose `fetch_method` is already `wayback`/`commoncrawl` are `ARCHIVED-ONLY` unless HEAD is 2xx. Throttle archive.org ≥1 s.
