...
5. **Extract 2-6 falsifiable claims per source** that bear on the research question. For each: a concrete checkable statement in your words, a **verbatim quote copied from the raw text** (one sentence or a contiguous span, 10-60 words), and importance `central` / `supporting` / `tangential`. Prefer numbers, dates, named entities, mechanisms and explicit findings over generalities. Register them in one call:
