...
| 3 Fetch + Extract | Fetch each selected URL through the keyless fallback chain (`scripts/fetch.py`), raw text to `raw/<n>.txt`. Extract **2-8 atomic falsifiable claims per source (one fact, ≤25 words), each with a verbatim quote**, source-quality grade, publish date. Append to `claims.json`; register source in `sources.md` with fetch method and evidence strength. No cap on claims; **central claims are capped mechanically at 4 per source and 16 per angle, and the researcher stops at the preset's source target** (2026-09-10; the count of central claims is the cost lever, see §2). | per angle (same agent as search, or fetch pool) | R1, R3, R5, R12, FETCH chain, OUT |
...
**Measured 2026-09-03 (Stage B probe, two `standard` runs):** verification took ~50% of generation tokens because researchers marked 70-90% of claims central regardless of prompt wording; the ledger now caps central claims at 4 per source (extra ones become supporting and are only quote-checked). Writers overshot length by 20-70%; the citation script now enforces the ceiling and the writer self-checks before finishing. Missed rubric items were adjacent normative rules on already-fetched pages; the researcher prompt now requires extracting every rule in cited sections. **Re-measured 2026-09-04 (two more `standard` runs, progress #38):** the per-source cap did not lower the central count (100 and 86) because researchers fetch about twice the source target, so verification stayed at ~60% of generation tokens; the writer self-check removed the separate fix passes but not the need for a coverage pass; one adjacent-statement miss remained (in a judgment). Compliance 1.00 and 0.90; cost 2.1-2.3M tokens per question. **Fifth run (2026-09-04, progress #39):** one prompt line, "stop at the source target; never exceed 5", made the per-source cap bind per run: 59 central claims, verification 0.97M, total 1.85M, compliance 1.00 with the coverage list already empty. **Applied 2026-09-10 (skill v1.1, progress #42):** the stop rule lives in `prompts/researcher.md`; the ledger adds a per-angle cap of 16 central claims (source target 4 × 4, env-overridable) as the mechanical backstop; researchers pass `--round` on every registration; verifiers grade the sources they add and the writer's confidence rule reads the grade; the writer outlines with per-section word budgets before drafting; the adjacent-rule extraction instruction covers judgments; the fetch gate rejects menu-only pages; `claim unevidence` is the ledger's one undo. **Confirmation run 2026-09-10 (culture-history-3, same brief):** compliance 1.00 again on the first pass with no low-grade-source criticism; cost 2.17M+ tokens because central claims rose to 80 (three angles hit the per-angle cap of 16 that the prompt named), so the numeral was removed from the prompt. **Re-test the same day (third paired run, progress #43):** with the cap stated but not numbered, central fell back to 63 (per angle 8-14, none at the cap), verification to 9 batches, cost to 1.85M tokens (the 2026-09-04 level; 1.90M with one presentation pass), compliance 0.966 on the first judgment (the dated timeline the question asks for was missing because the operator's writer fill did not request it) and 1.00 after that pass. The number in the prompt was the anchor; the per-angle cap default stays 16 as a backstop (`evidence/pilot-2026-09/README.md`, confirmation section). **Second domain (2026-09-10 evening, technology-3, progress #45):** v1.1 against the 2026-09-03 v1.0 run of the same brief: central 60 vs 85, 8 verifier batches vs 11, 1.64M tokens vs 1.50M, compliance 0.923 first pass vs 0.88 after a coverage pass, 39 min vs about 2 h; the presentation item passed from the writer-prompt rule alone; the one miss is the same adjacent rule on a fetched RFC page as in 2026-09-03 (`evidence/pilot-2026-09/README.md`, fuller-pilot section). **Third domain (the same evening, science-health-3, progress #47):** central 64 vs 91, 9 batches vs 13, 1.99M tokens vs 2.61M, compliance 0.893 first pass and 0.964 after one coverage pass vs 0.96 after two passes; both first-pass misses were facts held in `supporting` claims that the writer's coverage list (central only) does not surface, so widening that list is the next candidate; the remaining miss is on a page neither run fetched. **Fourth domain (2026-09-10/11, policy-law-3, progress #49, with the #48 supporting-claim list):** central 55 vs 86, 8 batches vs 11, 2.01M tokens vs 2.32M+, compliance 0.828 first pass and 0.897 after one fix pass (a writer inversion) vs 0.897; the same single miss as on 2026-09-04, a passage on a fetched judgment that three prompt rules have not made a researcher extract, so a mechanical question-coverage check before writing is the next candidate. **Fifth domain (2026-09-11, business-finance-3, progress #51, with the #50 question-coverage table):** central 73 vs 100, 10 batches vs 12, 1.87M tokens vs 2.10M plus six killed, compliance 0.92 first pass vs 0.92 first pass and 1.00 after a coverage pass; the coverage table was produced from the prompt alone and declared three gaps before judging, none of which was a rubric item; the one miss (the FDIC's institutional status, on a fetched page and in no claim) is a passage the question does not name, so the table's rows could not reach it, and the operator's re-decomposed round-2 brief had dropped the hypothesis line that made the 2026-09-04 researcher extract it.
