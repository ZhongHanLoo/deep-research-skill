...
- 2026-09-10: **Skill v1.1.** The seven changes queued from the pilot were applied (contracts v1.1 first, then `ledger.py`/`fetch.py`, prompts, tests; progress.md #42). Confirmation run of culture-history-3: compliance 1.00 again, cost up (80 central claims vs 59; 2.17M+ tokens vs 1.85M) because the prompt named the per-angle cap and researchers filled it; the numerals were removed from `prompts/researcher.md`, not yet re-tested. Re-test the same afternoon with the un-numbered prompt (progress.md #43, `eval/private/results/2026-09-10-B2/`): central 63, 1.85M tokens (1.90M with a presentation pass), compliance 0.966 first pass (the dated timeline was missing because the writer fill did not ask for it) and 1.00 after one pass. The cap default stays 16; **v1.1 tagged**. Next: the two small follow-ups in #43 (writer prompt: presentation elements from the brief; a way to clear a wrong `published` date), then decide on a fuller pilot.
