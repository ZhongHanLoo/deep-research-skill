...
- **Last action (2026-09-04, session 3, 00:15-02:05 BST):** ran `business-finance-3` (compliance 1.00 after a coverage pass, 2.10M tokens) and `policy-law-3` (0.897, 2.32M) with the three #36 fixes in place; verdict in #38: the fixes removed writer fix passes and one class of miss but did not cut cost (per-source cap is not binding; researchers over-fetch). Two `cite_check.py` bugs fixed (CDX-error → UNKNOWN health; legal-citation years). Run folders and tokens in `eval/private/results/2026-09-04-B/`.
- **Next action:** run `culture-history-3` per `eval/RUNBOOK.md` in a fresh quota window (not started: session budget was at the level where the limit hit twice). Then Stage C: user runs built-in `/deep-research` on the five stable questions → `eval/private/results/<date>-A/<id>/report.md`; audit with `eval/cite_audit.py`, Opus judges, `eval/score.py`. Design changes queued in #38 (per-angle central cap, prompt guards) are for after Stage C.
- **Session log:** session 1 ended 2026-09-02 after entry #27 (HEAD 7222c8e). Session 2 ran 2026-09-03 15:00-23:30 BST, entries #29-#36. Session 3 ran 2026-09-04 00:15-02:05 BST, entries #37-#38.
